...
var GitHubBot = class extends Service {
  ...
  async commitAndOpenPullRequest(input) {
    ...
    const baseRef = await this.request(input.org, `/repos/${owner}/${repo}/git/ref/heads/${encodeURIComponent(input.base)}`);
    const parentSha = baseRef.object.sha;
    const parent = await this.request(input.org, `/repos/${owner}/${repo}/git/commits/${parentSha}`);
