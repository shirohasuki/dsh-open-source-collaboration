		...
		ctx.tools.register(defineTool({
			...
			execute: async () => {
				return this.role.watchlist.map((repo) => {
					...
					return `${name}\t${existsSync(join(dir, ".git")) ? "ready" : "missing"}\t${dir}`;
				}).join("\n");
			}
		}));
	}
	repoRef(name) {
		const repo = this.role.watchlist.find((repo$1) => repo$1.split("/")[1] === name);
		if (repo === void 0) throw new Error("unknown repo: " + name);
		return repo;
	}
